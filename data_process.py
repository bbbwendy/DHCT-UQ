...
def get_data_loader(dataset, sensors, sequence_length, alpha, threshold, batch_size,train_size=0.9,random_state=42):
    dir_path = './data/'
    train_file = 'train_' + dataset + '.txt'
    test_file = 'test_' + dataset + '.txt'
    index_names = ['unit_nr', 'time_cycles']
...
class CNNModel(nn.Module):
    def __init__(self):
        ...
        # 第1层卷积
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=10, kernel_size=(10, 1), stride=(1, 1), padding='same')

        # 第2层卷积
        self.conv2 = nn.Conv2d(in_channels=10, out_channels=10, kernel_size=(10, 1), stride=(1, 1), padding='same')

        # 第3层卷积
        self.conv3 = nn.Conv2d(in_channels=10, out_channels=10, kernel_size=(10, 1), stride=(1, 1), padding='same')

        # 第4层卷积
        self.conv4 = nn.Conv2d(in_channels=10, out_channels=10, kernel_size=(10, 1), stride=(1, 1), padding='same')

        # 第5层卷积
        self.conv5 = nn.Conv2d(in_channels=10, out_channels=1, kernel_size=(3, 1), stride=(1, 1), padding='same')

        # 平滑层
        self.flatten = nn.Flatten()

        # 全连接层
        self.fc1 = nn.Linear(30 * 14, 100)
    ...
    def forward(self, x):
        # 假设你的数据在加载时是这样的形状: [batch_size, height, width, channels]
